...
| `node scripts/flow.mjs` | تأكيدات على مستوى الشاشة بـ Playwright: مسار الطلب كامل، العملة والوضع الداكن، سلوك لوحة العميل، إن كل عنصر تحكم بيعدّي 44px، إن مفيش «إثبات» متلفّق على صفحات الشركة، وإن كل الأرقام لاتينية. | 130 فحص، 129 ناجحة، كونسول نظيف. الفاشل «granting one shows it on the contact» جاي من master نفسه (PR #35) ومالوش علاقة بأي حاجة في الفرع ده — والسكريبت بيطبع FAIL وبيخرج بصفر، فالفحص الفاشل مابيوقّفش دمج لوحده |
...
- `locale.tsx` — جدول النصوص: 1,321 مفتاح، كل واحد `{ ar, en }`، بيتقري بـ `t('key')` ومفاتيحه
...
البوابات: 105 مسار، 130 فحص في `flow.mjs` منهم 129 ناجحة، 21 رحلة، 220 لقطة. الفحص
