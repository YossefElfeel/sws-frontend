...
**النسخة المنشورة:** <https://sws-frontend-mu.vercel.app/> — 100 مسار، عربي (الافتراضي)
وإنجليزي، فاتح وداكن.
...
| `node scripts/flow.mjs` | تأكيدات على مستوى الشاشة بـ Playwright: مسار الطلب كامل، العملة والوضع الداكن، سلوك لوحة العميل، إن كل عنصر تحكم بيعدّي 44px، إن مفيش «إثبات» متلفّق على صفحات الشركة، وإن كل الأرقام لاتينية. | 125 فحص، كونسول نظيف |
...
| `node scripts/mobile.mjs` | كل المسارات على 390px بالعربي: تجاوز أفقي، أهداف أقل من 44px، عناصر أقرب من 8px لبعض، نص أصغر من 12px، عناصر مغطية، أيقونات معصورة. | 100/100 مسار نظيف |
| `node scripts/capture.mjs` | صورة كاملة لكل مسار على 1440 و390 في `.impeccable/review/`، وبيفشل على تجاوز أفقي أو `main` فاضية أو أي خطأ في الكونسول. | 210 لقطة نظيفة |
| `scripts/routes.mjs` | مش بوابة — ده **الجدول الوحيد** للمسارات، اللي `capture.mjs` و`mobile.mjs` بيقروا منه. أي مسار جديد لازم يتضاف هنا وإلا محدش هيصوّره ولا يفحصه. | 100 مسار |
...
- `locale.tsx` — جدول النصوص: 1,298 مفتاح، كل واحد `{ ar, en }`، بيتقري بـ `t('key')` ومفاتيحه
...
الست كلها ناجحة: 100 مسار، 125 فحص في `flow.mjs`، 21 رحلة، 210 لقطة.
