...
**النسخة المنشورة:** <https://sws-frontend-mu.vercel.app/> — 105 مسار، عربي (الافتراضي)
وإنجليزي، فاتح وداكن.
...
عن `tokens.json`، وبيفشل تاني على أي سقوط في WCAG 2.2 AA (84 فحص، كلها ناجحة دلوقتي).
...
| `node scripts/flow.mjs` | تأكيدات على مستوى الشاشة بـ Playwright: مسار الطلب كامل، العملة والوضع الداكن، سلوك لوحة العميل، إن كل عنصر تحكم بيعدّي 44px، إن مفيش «إثبات» متلفّق على صفحات الشركة، وإن كل الأرقام لاتينية. | 141 فحص، 140 ناجحة، كونسول نظيف. الفاشل «granting one shows it on the contact» جاي من master نفسه (PR #35) ومالوش علاقة بأي حاجة في الفرع ده — والسكريبت بيطبع FAIL وبيخرج بصفر، فالفحص الفاشل مابيوقّفش دمج لوحده |
...
| `node scripts/mobile.mjs` | كل المسارات على 390px بالعربي: تجاوز أفقي، أهداف أقل من 44px، عناصر أقرب من 8px لبعض، نص أصغر من 12px، عناصر مغطية، أيقونات معصورة. | 105/105 مسار نظيف |
| `node scripts/capture.mjs` | صورة كاملة لكل مسار على 1440 و390 في `.impeccable/review/`، وبيفشل على تجاوز أفقي أو `main` فاضية أو أي خطأ في الكونسول. | 220 لقطة نظيفة |
| `scripts/routes.mjs` | مش بوابة — ده **الجدول الوحيد** للمسارات، اللي `capture.mjs` و`mobile.mjs` بيقروا منه. أي مسار جديد لازم يتضاف هنا وإلا محدش هيصوّره ولا يفحصه. | 105 مسار |
...
| `TableToolbar` | الشريط اللي كل جدول في لوحة العميل بيلبسه: خانة بحث واحدة والفلاتر جنبها كـ selects. معاه `TableCount` — عدّاد «كام من كام» بيتحط تحت الجدول نفسه مش تحت خانة البحث. |
| `Select` | القايمة المنسدلة بإطار الحقول نفسه: سهم الكروم الافتراضي بيتشال، وبدله `IconChevron` على بُعد 16px من الحافة زي النص على الناحية التانية. |
...
- `products.ts` — باقي العائلات: ووردبريس، سحابي، بريد، VPS (مواصفات السيرفرات + أنظمة
  التشغيل، والكروت مشتقة من نفس الأرقام)، مراقبة، SSL، منشئ المواقع. كل العائلات بقت كروت زي
  الاستضافة المشتركة — الـ VPS كان آخر جدول وراح.
...
- `accountState.tsx` — النسخة القابلة للتعديل من الفيكستشرز. صفحات الدومين الثمانية وصفحة
  الخدمة والفواتير بيقروا ويكتبوا هنا، فقفل اتفتح في صفحة بيتقري مفتوح في اللي بعدها، وفاتورة
  اتلغت من القائمة بتختفي من «محتاج منك حاجة» ومن عدّاد القايمة الجانبية في نفس اللحظة. مفيش
  حفظ بعد إعادة التحميل — عن قصد، عشان البروتوتايب مايتراجعش كأن وراه سيرفر.
- `cart.tsx` — السلة: السطر فيه الباقة والدورة والإضافات، والدومين المربوط بإضافاته المجانية،
  وإعدادات السيرفر لو VPS.
- `locale.tsx` — جدول النصوص: 1,426 مفتاح، كل واحد `{ ar, en }`، بيتقري بـ `t('key')` ومفاتيحه
...
- **جدول جديد في لوحة العميل:** يلبس `TableToolbar` زي كل الجداول، ويتحط جوه `.table-scroll`
  عشان يتلف جوه الكارت على الموبايل بدل ما يوسّع الصفحة، و`TableCount` تحته — بـ`inset` لو
  الشريط نفسه جوه الكارت، من غيرها لو الشريط فوق الكارت. العدّاد بيمشي مع الشريط مش مع الجدول.
- **قايمة منسدلة جديدة:** `<Select>` مش `<select className="field">`، عشان السهم والمسافة
  اللي جنبه يبقوا واحدين في كل الشاشات.
...
**2026-09-15** — الفواتير: حالتين كانتا في الفلتر ومفيش صف في الحساب بيوصلهما — «متأخرة»
و«ملغاة» — بقى ليهما فيكستشر. المتأخرة مدفوعة جزئيًا (5.00 من 8.55)، وده اللي خلّى «المتبقي
للدفع» حساب على سجل الدفع مش نسخة تانية من الإجمالي: كارت «محتاج منك حاجة» ولوحة القيادة
بقوا يجمعوا المتبقي زي ما شاشات الدفع بتعمل من الأصل. والملغاة كشفت إن الشاشة كانت بتقول
عليها «مدفوعة بالكامل» — بقى ليها حالتها التالتة اللي بتقول إن مفيش حاجة مستحقة ومفيش حاجة
اتدفعت. وصف الفواتير خد قائمة إجراءات (`RowMenu`) فيها PDF وتعديل وإلغاء وشيل، كل واحدة
بتظهر على الحالة اللي ليها معنى فيها بس، والاتنين اللي بياخدوا حاجة بيسألوا تاني في مكانهم
قبل ما يتنفذوا — وكلها معلَّمة بـ`DevNote` لأن التلاتة إجراءات إدارية في WHMCS.

البوابات: 105 مسار، 141 فحص في `flow.mjs` منهم 140 ناجحة، 21 رحلة، 220 لقطة. الفحص
الفاشل الوحيد على تصاريح جهة الاتصال وجاي من master نفسه — اتأكدنا إنه بيفشل على
`origin/master` لوحده بنفس الأرقام.
