...
export function Dashboard() {
  ...
  return (
    ...
    >
       ...
       */}
      <div className="dash dash--flow">
        {/*
         * 2026-09-16: two columns that pack, rather than ten cards on shared rows.
         *
         * `--paired` put the cards straight into the grid so both columns shared row tracks,
         * which did rule the page: every pair began on one line. It also made every row as
         * tall as the taller of its two cards, and the shorter one kept the difference —
         * 477px of it down the screen, 142px of that in one gap under the credit card.
         *
         * Stretching the short card to fill its row is what the ruling cost, and giving the
         * height back as a gap is what it costs once the card stops stretching. There is no
         * third answer in the grid: masonry is what packs columns independently, and no engine
         * this runs on has it. So the columns become real again, each one flowing at its own
         * pace, and the page ends 162px sooner with no hole in it.
         *
         * Below 1280 the two wrappers go `display: contents` and the cards interleave back
         * into one column by `order` — the phone reads what it read before, which is the one
         * thing the wrappers would otherwise have taken away.
         */}
        <div className="dash__main">
          {/* The only thing on this screen that is genuinely owed. */}
          {unpaid.length > 0 ? (
          ...
          {/* The services read as rows rather than a table: three columns of data do not
              need a table's machinery, and rows survive a narrow column. */}
          <Card
          ...
          {/*
           * The site's own two questions — "have I run out of space" and "where is the file
           * manager" — which this screen used to send people to the service page to answer.
           *
           * They report ONE service, the primary hosting account named on both cards, rather
           * than a total across the account: a disk bar summed over three products is a
           * number with no action attached to it, since you cannot clear space on "72% of
           * everything". Absent when there is no live cPanel service, because a meter at zero
           * over a product the account does not have reads as a broken card.
           *
           * The shortcuts take the wide column and the meters the narrow one, which is the
           * way round their content wants. `.shortcuts` lays itself out in 11rem tracks and
           * the narrow column is 359px inside its padding — one pixel short of two tracks and
           * the gap between them, so the ten links fell into a single column ten rows tall.
           */}
          {primary && (
          ...
          {/* The screen ends on support: what is already open with us. */}
          <Card
        ...
        <div className="dash__side">
          ...
          {/* What is sitting in the account to pay for it. */}
          ...
          {/* …beside the door that opens the next one. */}
